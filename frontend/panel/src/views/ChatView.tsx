    ...
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-bold text-zinc-900 tracking-tight flex items-center gap-3">
          <MessageSquare className="text-violet-600" size={26} /> CX Bot — Simulador de WhatsApp
        </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className={`${card} p-4 space-y-3`}>
          <h3 className="text-sm font-bold text-zinc-800 uppercase tracking-widest">Conversaciones</h3>
          <div className="space-y-2 border-b border-zinc-200 pb-3">
            ...
            <button onClick={() => setSelected(null)}
              className="w-full py-2 bg-violet-50 border border-violet-300 text-violet-600 rounded-xl text-xs font-bold">
              + Nueva conversación con estos datos
            </button>
          </div>
          {conversations.map((c) => (
            <div key={c.id} onClick={() => setSelected(c)}
              className={`p-3 rounded-xl border cursor-pointer ${selected?.id === c.id ? "bg-violet-50 border-cyan-500/50" : "bg-zinc-50 border-zinc-200 hover:bg-zinc-50"}`}>
              <div className="flex justify-between items-center">
                <p className="font-bold text-sm text-zinc-900">{c.contact_name || c.contact_phone}</p>
                {c.status === "needs_human" && (
                  <span className="text-[9px] px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-500/20 font-bold uppercase">Humano</span>
                )}
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {messages.map((m, i) => (
              <div key={`${m.id}-${i}`} className={`flex ${m.direction === "in" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[75%] px-3.5 py-2 rounded-2xl text-sm whitespace-pre-wrap ${m.direction === "in" ? "bg-cyan-600/30 text-cyan-50 rounded-br-sm" : "bg-zinc-100 text-zinc-800 rounded-bl-sm"}`}>
                  {m.body}
                </div>
              </div>
            ))}
            {sending && (
              <div className="flex justify-start">
                <div className="bg-zinc-100 text-zinc-500 px-3.5 py-2 rounded-2xl text-sm italic">escribiendo…</div>
              </div>
            )}
            {!messages.length && !sending && (
              <p className="text-center text-zinc-500 text-xs pt-24">
                {selected ? "Sin mensajes." : "Cargá teléfono y nombre a la izquierda y escribí el primer mensaje como si fueras el cliente."}
              </p>
            )}
          </div>
          {error && <p className="text-red-600 text-xs px-4 pb-1">{error}</p>}
