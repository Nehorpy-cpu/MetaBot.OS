    ...
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-bold text-zinc-900 tracking-tight flex items-center gap-3">
          <Activity className="text-violet-600" size={26} /> Inteligencia del Enjambre
        </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className={`lg:col-span-2 ${card} p-5 space-y-3`}>
          <h3 className="text-sm font-bold text-zinc-800 uppercase tracking-widest">Informes ({reports.length})</h3>
          {!reports.length && <p className="text-xs text-zinc-500">Todavía no hay informes. Generá el primero.</p>}
          {reports.map((r) => (
            <div key={r.id} onClick={() => setOpenReport(r)}
              className="p-3 rounded-xl border bg-zinc-50 border-zinc-200 hover:bg-zinc-50 cursor-pointer flex justify-between items-center">
              <div>
                <p className="font-bold text-sm text-zinc-900">{r.title}</p>
                <p className="text-[10px] text-zinc-500">{new Date(r.created_at).toLocaleString("es-PY")}</p>
              </div>
              <span className="text-[9px] px-2 py-0.5 rounded-full border border-zinc-200 bg-zinc-50 text-zinc-600 font-bold uppercase">
                {r.kind === "weekly" ? "Quant" : "Competencia"}
              </span>
            </div>
          ))}

          <h3 className="text-sm font-bold text-zinc-800 uppercase tracking-widest pt-4">
            Mejoras de prompts propuestas ({suggestions.filter((s) => s.status === "pending").length} pendientes)
          </h3>
          {suggestions.filter((s) => s.status === "pending").map((s) => (
            <div key={s.id} className="p-3 rounded-xl border bg-zinc-50 border-zinc-200 space-y-2">
              <div className="flex justify-between items-center">
                <p className="font-bold text-sm text-zinc-900">{s.agent_name}</p>
                <div className="flex gap-2">
                  <button onClick={() => setOpenSuggestion(s)}
                    className="text-xs bg-zinc-50 hover:bg-zinc-200 border border-zinc-200 px-3 py-1 rounded-lg text-zinc-700">Ver</button>
                  <button onClick={() => run("apply", () => intelApi.applySuggestion(companyId, s.id))}
                    className="text-xs bg-emerald-50 hover:bg-emerald-100 border border-emerald-300 px-3 py-1 rounded-lg text-emerald-700">Aplicar</button>
                  <button onClick={() => run("reject", () => intelApi.rejectSuggestion(companyId, s.id))}
                    className="text-xs bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 px-3 py-1 rounded-lg text-red-600">Rechazar</button>
                </div>
              </div>
              <p className="text-xs text-zinc-600">{s.rationale}</p>
            </div>
          ))}
          {!suggestions.filter((s) => s.status === "pending").length && (
            <p className="text-xs text-zinc-500">Sin mejoras pendientes. El Optimizador corre solo los sábados o con el botón de arriba.</p>
          )}

          <h3 className="text-sm font-bold text-zinc-800 uppercase tracking-widest pt-4">Hallazgos del Auditor ({findings.length})</h3>
          {!findings.length && <p className="text-xs text-zinc-500">Sin hallazgos. El Guard no encontró problemas (o aún no corrió).</p>}
          {findings.map((f) => (
            <div key={f.id} className="p-3 rounded-xl border bg-zinc-50 border-zinc-200">
              ...
              <p className="text-xs text-zinc-700">{f.note}</p>
            </div>
          ))}
        </div>

        <div className={`${card} p-5 space-y-3`}>
          <h3 className="text-sm font-bold text-zinc-800 uppercase tracking-widest">Competidores ({competitors.length})</h3>
          ...
          <button disabled={!newUrl.trim() || !!busy}
            ...
            className="w-full py-2 bg-violet-50 border border-violet-300 text-violet-600 rounded-xl text-xs font-bold disabled:opacity-40">
            + Agregar competidor
          </button>
          {competitors.map((c) => (
            <div key={c.id} className="p-2.5 rounded-xl border bg-zinc-50 border-zinc-200 flex justify-between items-center">
              <div className="min-w-0">
                <p className="text-xs font-bold text-zinc-800 truncate">{c.label || c.url}</p>
                <p className="text-[10px] text-zinc-500 truncate">{c.url}</p>
              </div>
              <button onClick={() => run("delcomp", () => intelApi.deleteCompetitor(companyId, c.id))}
                className="text-zinc-500 hover:text-red-600 p-1 shrink-0"><X size={14} /></button>
            </div>
          ))}
        </div>
      </div>
      ...
      {openReport && (
        <Modal title={openReport.title} onClose={() => setOpenReport(null)}>
          <pre className="whitespace-pre-wrap text-sm text-zinc-800 font-sans leading-relaxed">{openReport.content}</pre>
        </Modal>
      )}
      {openSuggestion && (
        <Modal title={`Mejora propuesta: ${openSuggestion.agent_name}`} onClose={() => setOpenSuggestion(null)}>
          <div className="space-y-4 text-sm">
            <div>
              <p className="text-xs font-bold text-zinc-600 uppercase tracking-widest mb-2">Motivo</p>
              <p className="text-zinc-700">{openSuggestion.rationale}</p>
            </div>
            <div>
              <p className="text-xs font-bold text-zinc-600 uppercase tracking-widest mb-2">Prompt actual</p>
              <pre className="whitespace-pre-wrap text-xs bg-[#f6f4ff] p-3 rounded-lg border border-zinc-200 text-zinc-600">{openSuggestion.old_prompt}</pre>
            </div>
            <div>
              <p className="text-xs font-bold text-zinc-600 uppercase tracking-widest mb-2">Prompt propuesto</p>
              <pre className="whitespace-pre-wrap text-xs bg-[#f6f4ff] p-3 rounded-lg border border-cyan-500/20 text-cyan-100">{openSuggestion.suggested_prompt}</pre>
            </div>
          </div>
        </Modal>
      )}
