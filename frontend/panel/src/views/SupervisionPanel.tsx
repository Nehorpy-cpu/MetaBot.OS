...
export function SupervisionPanel({ company, onChanged }: { company: Company; onChanged: () => void }) {
  ...
  return (
    <div className={`${card} p-6 space-y-5`}>
      <div>
        <h3 className="text-lg font-bold text-white">Supervisión del CEO</h3>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2 mb-1.5">
                <Icono size={16} className={activo ? "text-cyan-400" : "text-zinc-500"} />
                <span className={`text-sm font-bold ${activo ? "text-cyan-300" : "text-zinc-200"}`}>
                  {m.titulo}
                </span>
              </div>
              <p className="text-[11px] leading-relaxed text-zinc-400">{m.detalle}</p>
            </button>
          );
        })}
      </div>
      ...
      {report && report.total > 0 ? (
        <div className="space-y-4 pt-4 border-t border-white/5">
          ...
          <div>
            <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-2">Qué encontró</p>
            <div className="space-y-1.5">
              {Object.entries(report.por_disparador)
                .sort((a, b) => b[1] - a[1])
                .map(([clave, cantidad]) => (
                  <div key={clave} className="flex items-center justify-between text-sm">
                    <span className="text-zinc-300">{DISPARADORES[clave] ?? clave}</span>
          ...
          {report.recientes.length > 0 && (
            <div>
              <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-2">Últimas revisiones</p>
              <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                {report.recientes.map((e) => (
                  <div key={e.id} className="text-xs bg-white/[0.02] border border-white/5 rounded-lg p-3">
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-zinc-200 font-medium">{DISPARADORES[e.trigger] ?? e.trigger}</span>
        ...
        <p className="text-xs text-zinc-500 pt-4 border-t border-white/5">
          {modoActual === "off"
...
function Metrica({ etiqueta, valor, nota }: { etiqueta: string; valor: string; nota?: string }) {
  return (
    <div>
      ...
      <p className="text-[11px] text-zinc-400 mt-0.5">{etiqueta}</p>
      {nota && <p className="text-[10px] text-zinc-600 mt-0.5 leading-tight">{nota}</p>}
    </div>
  );
}
